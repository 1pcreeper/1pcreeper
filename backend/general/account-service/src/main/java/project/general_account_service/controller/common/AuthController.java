...
public class AuthController {
    ...
    public ResponseEntity<APIBaseResponseDTO<AppUserVerifyResponseDTO>> verify(
        ...
        Set<String> outhRoles;
        try{
            outhRoles = firebaseAuthService.getRoles(appUser.getUid());
        }catch (FirebaseAuthException e){
            throw new PrincipalException("Failed to get System Roles info");
        }
        if(Objects.isNull(outhRoles)){
            outhRoles = new HashSet<>();
        }
        return ResponseEntity.ok().body(
            APIBaseResponseDTO.success(
                appUserMapper.toAppUserVerifyResponseDTO(appUser,outhRoles)
            )
        );
    }
}
